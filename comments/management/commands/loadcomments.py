...
class Command(BaseCommand):
    ...
    def _create_comments(self, comments, users):
        ...
        for comment_info in comments:
            text = lorem.sentence() if self.lorem_text \
                else comment_info['text']

            user = users[comment_info['user']]
            product = products[comment_info['product']]
            parent = db_comments[comment_info['parent']] if comment_info['parent'] else None

            comment, created = Comment.objects.get_or_create(
                timestamp = make_aware(datetime.datetime.strptime(comment_info['timestamp'], "%Y-%m-%d %H:%M:%S")),
                defaults = {
                    'user': user,
                    'product': product,
                    'text': text,
            ...
            if created:
                if not comment.parent:
                    print(f'Created comment by {comment.user.username} for {comment.product.name}')
                else:
                    print(f'Created reply by {comment.user.username} in {comment.product.name} comments')
